...
def smooth():
    ...
    use_pae_calibration = body.get("usePAECalibration", False)
    slider_level = body.get("sliderLevel", None)  # 0-100 value from slider

    s = load_series(sid)
    y = s.get("y", [])
    
    # If PAE calibration is enabled and slider level is provided, recalculate params
    if use_pae_calibration and slider_level is not None:
        from .pae_calibration import calibrate_parameter_for_pae
        try:
            params = calibrate_parameter_for_pae(y, method, slider_level)
        except Exception as e:
            # Fall back to original params if calibration fails
            print(f"PAE calibration failed: {e}, using original params")
